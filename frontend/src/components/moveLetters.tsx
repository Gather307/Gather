...
const MoveLetters: React.FC = () => {
  const letters = 'GATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHERGATHER'.split('');
  const sceneRef = useRef<HTMLDivElement>(null);
  const [positions, setPositions] = useState<{ x: number; y: number; vx: number; vy: number; settled: boolean }[]>([]);
      ...
      setPositions((prevPositions) => {
        ...
        newPositions.forEach((pos, index) => {
          if (!pos.settled) {
            ...
            if (buttonRect) {
              const distanceToButton = Math.sqrt(Math.pow(newX - (buttonRect.left + buttonRect.width / 2), 2) + Math.pow(newY - (buttonRect.top + buttonRect.height / 2), 2));
              if (distanceToButton < 150) { // Adjust the distance as needed
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    setPositions(prevPositions => {
      ...
      newPositions.forEach(pos => {
        ...
        if (distance < 500) { // Adjust the range as needed
    ...
    const handleMouseMovePull = () => {
        ...
        setPositions(prevPositions => {
          ...
          newPositions.forEach(pos => {
            ...
            if (distance < 5000) { // Adjust the range as needed
                pos.vx = dx / distance * speed;
                pos.vy = dy / distance * speed;
            }
    ...
    const handleButtonClick = () => {
      ...
      return (
        <div className="letterwrapper">
          <div className="letter-container" ref={sceneRef} onMouseMove={handleMouseMove}>
            {positions.map(({ x, y }, index) => (
              <div key={index} className="letter" style={{ left: `${x}px`, top: `${y}px` }}>
                {letters[index]}
              </div>
            ))}
          </div>
